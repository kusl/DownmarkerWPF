(function() {
    ...
    window._pr_isIE6 = function() {
        ...
        window._pr_isIE6 = function() { return K };
        return K
    };
    var ba = "a",
        ca = "z",
    ...
    (function() {
        ...
        function Qc(b) { return b >= ba && b <= ca || b >= da && b <= ea }
        ...
        function Tc(b) { return b.replace(wa, ub).replace(xa, vb).replace(ya, wb).replace(Sc, xb) }

        function Z(b) { return b.replace(wa, ub).replace(xa, vb).replace(ya, wb) }
        ...
        function za(b) { return Cb === b.tagName }
        ...
        function R(b, a) {
            ...
            case 1:
                ...
                if (b.firstChild ||
                    !/^(?:br|link|img)$/.test(d))a.push(Db, d, B);
                break;
            case 2:
        ...
        var id = J([], [
                [I, /^[^<?]+/, j], [
                    Kb, /^<!\w[^>]*(?:>|$)/,
                    j
                ], [P, /^<!--[\s\S]*?(?:--\>|$)/, j], [L, /^<\?([\s\S]+?)(?:\?>|$)/, j], [L, /^<%([\s\S]+?)(?:%>|$)/, j], [S, /^(?:<[%?]|[%?]>)/, j], [L, /^<xmp\b[^>]*>([\s\S]+?)<\/xmp\b[^>]*>/i, j], [Lb, /^<script\b[^>]*>([\s\S]+?)<\/script\b[^>]*>/i, j], [Mb, /^<style\b[^>]*>([\s\S]+?)<\/style\b[^>]*>/i, j], [T, /^<\/?\w[^<>]*>/, j]
            ]),
            jd = /^(<[^>]*>)([\s\S]*)(<\/[^>]*>)$/;
        ...
        var ld = J([[U, /^\'[^\']*(?:\'|$)/, j, Ab], [U, /^\"[^\"]*(?:\"|$)/, j, C], [S, /^[<>\/=]+/, j, Nb]], [[T, /^[\w:\-]+/, /^</], [U, /^[\w\-]+/, /^=/], [V, /^[\w:\-]+/, j], [I, /^\s+/, j, Ob]]);
        ...
        function y(b) {
            ...
            b.regexLiterals && d.push([
                W, /^\/(?=[^\/*])(?:[^\/\x5B\x5C]|\x5C[\s\S]|\x5B(?:[^\x5C\x5D]|\x5C[\s\S])*(?:\x5D|$))+(?:\/|$)/,
                Rc
            ]);
            var c = K(b.keywords), g = J(a, d), e = J([], [[I, /^\s+/, j, Sb], [I, /^[a-z_$@][a-z_$@0-9]*/i, j], [X, /^0x[a-f0-9]+[a-z]/i, j], [X, /^(?:\d(?:_\d+)*\d*(?:\.\d*)?|\.\d+)(?:e[+\-]?\d+)?[a-z]*/i, j, Tb], [S, /^[^\s\w\.$@]+/, j]]);
            ...
            function h() {
                ...
                for (; f < c.length && (new Date).getTime() < q; f++) {
                    ...
                    if (i.className && i.className.indexOf(Mc) >= 0) {
                        ...
                        if (o)o = o[1];
                        var r = false;
                        for (var l = i.parentNode; l; l = l.parentNode)
                            if ((l.tagName === Jc || l.tagName === Kc || l.tagName === Lc) && l.className && l.className.indexOf(Mc) >= 0) {
                                r = n;
                                break
                            }
                        if (!r) {
